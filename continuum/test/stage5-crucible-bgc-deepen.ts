/**
 * continuum/test/stage5-crucible-bgc-deepen.ts
 *
 * Stage 5 — Crucible Session 1 — Deepen BG-C with worked example
 */
...
const MESSAGE = `Deepen BG-C. I want a worked example that makes the taxonomy-fields-metrics-traceability framework concrete. Give me a real diagnostic scenario where I have to use all four layers to find the problem.`;
...
async function main() {
  ...
  const connectRes = await request("connect", {
    ...
    client: {
      id: CLIENT_ID,
      displayName: "Stage5 Crucible BG-C Deepen",
      version: "dev",
      platform: CLIENT_PLATFORM,
      mode: CLIENT_MODE,
      instanceId: "stage5-crucible-bgc-deepen",
    },
    locale: "en-US",
    userAgent: "stage5-crucible-bgc-deepen",
    role: "operator",
    scopes: [...CONNECT_SCOPES],
    caps: [],
    auth: { token: TOKEN },
  ...
  const acceptedRes = await request(
    "agent",
    { sessionKey: "agent:crucible:main", idempotencyKey, message: MESSAGE },
    15_000,
  );
