/**
 * continuum/test/stage5-crucible-q6-answer.ts
 *
 * Stage 5 — Crucible Session 1 — Faheem's answer to Question 6
 */
...
const ANSWER = `1. Calibration test: Have the agent answer a set of tasks and attach a confidence score to each answer, then compare confidence to actual correctness or outcome quality across the set. A calibrated agent's confidence should track reality. If it is frequently high-confidence and wrong, it is overcalibrated. If low-confidence and right, undercalibrated.

2. Reflection loop after failures:
- detect the failure
- classify the failure type
- inspect what evidence was actually used
- compare the chosen reasoning path to the missed better path
- generate one corrective rule or retrieval update candidate
- test that correction on a similar case before promoting it

3. An assessment is evidence-based when it is grounded in observable artifacts — demonstrated outputs the learner produced, not impressions of how fluent or confident they sounded.

4. A polished agent can sound helpful while being wrong, unsafe, or poorly grounded. Evaluating only by user satisfaction or style creates false trust: the system appears improved because it feels better to interact with, even though calibration, correctness, and governance performance may have degraded. Style is a surface signal. It can mask structural failure.`;
...
async function main() {
  ...
  const connectRes = await request("connect", {
    ...
    client: {
      id: CLIENT_ID,
      displayName: "Stage5 Crucible Q6 Answer",
      version: "dev",
      platform: CLIENT_PLATFORM,
      mode: CLIENT_MODE,
      instanceId: "stage5-crucible-q6",
    },
    locale: "en-US",
    userAgent: "stage5-crucible-q6",
    role: "operator",
    scopes: [...CONNECT_SCOPES],
    caps: [],
    auth: { token: TOKEN },
