...
## ADR-036 — TAR-009 Activation for Signal External Retrieval Boundary

**Date:** 2026-03-21
**PENDING entries resolved:** tar-enforcement.ts 13:18:08, tar-web-retrieval-enforcement.ts 13:18:24, continuum-governance-plugin.ts 13:18:28, continuum-governance-plugin.ts 13:18:32, continuum-governance-plugin.ts 13:18:38, tar-web-retrieval-enforcement.test.ts 13:18:57

**What changed:**
Four files were modified or created to activate TAR-009 as a single cohesive capability authorization unit:

- `tar-enforcement.ts` — TAR-009 entry added to `ACTIVE_TAR_REGISTRY`:
  - `capability_id: "TAR-009"`, `mcp_server_name: "web"`, `operation_names: ["web_search", "web_fetch"]`
  - `action_class: "read-only"`, `approved_agent_ids: ["signal"]`, `status: "active"`, `grant_required: false`
  - `mcp_server_name: "web"` is a virtual server identifier. The underlying tools (`web_search`, `web_fetch`) are native OpenClaw tools from the `tools.web` capability group (confirmed in `src/agents/tool-catalog.ts`, `sectionId: "web"`), not MCP-prefixed tools. The `"web"` server name follows the same registry convention used for `"filesystem"` and `"obsidian-vault"`, enabling `resolveCapability()` and `checkAgentAuthorization()` to operate uniformly across MCP and native surfaces.

- `tar-web-retrieval-enforcement.ts` — new file. `tarSignalWebRetrievalBeforeToolCallHandler`: intercepts `web_search` and `web_fetch` native tool calls before execution. For Signal: calls `enforceBeforeToolCall({ serverName: "web", operationName: event.toolName, ... })` → allowed. For any other agent: denied, `{ block: true, blockReason }` returned, INTEGRITY event emitted. Non-web tools pass through immediately. A dedicated handler was required because `tarBeforeToolCallHandler` in `tar-hook.ts` returns early for non-MCP tools (`parseMcpToolName()` returns `{ isMcp: false }` for tool names that do not begin with `mcp__`). Web native tools are structurally outside the MCP enforcement path and must be governed by a separate hook.

- `continuum-governance-plugin.ts` — three changes comprising one wiring concern: (1) import of `tarSignalWebRetrievalBeforeToolCallHandler` added; (2) `api.on("before_tool_call", tarSignalWebRetrievalBeforeToolCallHandler, { priority: 93 })` registered after TAR-007 at priority 94; (3) activation log string updated from `TAR-001-008` to `TAR-001-009`. Three separate PENDING entries were generated by three sequential edits to the same file; they are one wiring concern.

- `tar-web-retrieval-enforcement.test.ts` — new test file. Seven tests: Signal allowed on `web_search` (registry + hook), Signal allowed on `web_fetch` (registry), non-Signal agents denied on `web_search` and `web_fetch` with INTEGRITY event (foundry, the-bridge via registry; foundry via hook), non-web tool passes through hook without blocking.

**Why grouped as one ADR:**
All six PENDING entries implement a single architectural decision: activating TAR-009 as Signal's authorized external retrieval capability. The registry entry, the enforcement handler, the plugin wiring, and the test coverage are the four inseparable components of a single TAR activation. They have no independent architectural meaning — a registry entry without a handler is unenforced; a handler without a registry entry has no capability to resolve; wiring without tests leaves the enforcement surface unvalidated. They are one decision, committed as one deliverable.

**Why smallest safe patch:**
TAR-009 is scoped to exactly the tools Signal requires for external retrieval: `web_search` and `web_fetch`. No other agent's tool surface is widened. No existing TAR entry is modified. The handler fires only on those two tool names and returns immediately for all others. `grant_required: false` (envelope-allowed) is correct for Signal's retrieval cycle, which operates within a Bridge-defined scope envelope rather than requiring per-invocation grant authorization. No filesystem-style path check is needed because web retrieval has no path concept; for TAR-009 the minimal enforcement gate is agent identity plus the Bridge-defined retrieval envelope. The three `continuum-governance-plugin.ts` edits are three sequential incremental changes to one file — they are absorbed into a single ADR entry rather than three.

**Alternatives rejected:**
Two alternatives were considered and rejected:

1. _Extending `tarBeforeToolCallHandler` to cover native web tools_ — modifying the existing MCP handler in `tar-hook.ts` to also intercept non-MCP tools. Rejected because `tarBeforeToolCallHandler` is architecturally scoped to MCP tools: it uses `parseMcpToolName()` as its gate and passes non-MCP tools through by design. Widening its scope would conflate two distinct governance surfaces (MCP capability authorization and native tool authorization) in a single handler, making the enforcement boundary ambiguous and harder to reason about independently. A dedicated handler is the correct pattern, consistent with how `tar-read-enforcement.ts` handles the native `read` tool separately from its MCP counterparts.

2. _Grant-required authorization for Signal web retrieval_ — requiring an explicit per-invocation grant rather than envelope-allowed access. Rejected because Signal's retrieval cycle operates within a scope envelope defined by The Bridge; the authorization boundary is the agent identity, not an individual call grant. Introducing grant-required authorization for a read-only retrieval surface would add ceremony without adding control, since the same Bridge scope definition that governs Signal's cycle also constrains what Signal may retrieve.
